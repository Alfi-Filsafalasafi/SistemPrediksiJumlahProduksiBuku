    ...
    public function generate_fuzzy()
    {
        ...
        if ($this->form_validation->run() === FALSE) {
            ...
            if ($data['newpermintaan'] <= $data['min_permintaan']) {
                ...
                $data['permintaan_naik'] = ($data['newpermintaan'] - $data['min_permintaan']) / ($data['max_permintaan'] 
                - $data['min_permintaan']);
            }
            // permintaan turun
            if ($data['newpermintaan'] <= $data['min_permintaan']) {
                ...
                $data['permintaan_turun'] = ($data['max_permintaan'] - $data['newpermintaan']) / ($data['max_permintaan'] 
                - $data['min_permintaan']);
            }
            ...
            // ------------ defuzzifikasi --------------
            $data['z_defuzzifikasi'] = ($data['a_rules_1'] * $data['z_rules_1'] + $data['a_rules_2'] * $data['z_rules_2'] + 
                                        $data['a_rules_3'] * $data['z_rules_3'] + $data['a_rules_4'] * $data['z_rules_4']) / 
                                        ($data['a_rules_1'] + $data['a_rules_2'] + $data['a_rules_3'] + $data['a_rules_4']);
            $data['hasil_defuzzifikasi'] = round($data['z_defuzzifikasi']); // pembulatan bilangan dan hasil akhir
